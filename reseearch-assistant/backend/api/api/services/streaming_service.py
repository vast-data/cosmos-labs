    ...
    def create_tool_hook(self, timing_metrics: Dict[str, Any]) -> Callable:
        ...
        def tool_hook(function_name: str, function_call: Callable, arguments: Dict[str, Any]):
            ...
            try:
                ...
                # Add collection metadata for retrieve_chunks
                if function_name == "retrieve_chunks":
                    collection_metadata = None
                    if isinstance(result, dict):
                        collection_metadata = result.get("collection_metadata")
                    
                    # If metadata not found in result (e.g., result is a list), get it from arguments
                    if not collection_metadata:
                        try:
                            # Try to get collection name from arguments
                            # The collection_name might be in arguments, but it's usually empty since we use target_collection
                            # We need to get it from the agent's collection_name or from the tool closure
                            # For now, let's try to get it from the streaming service's collection_name if available
                            ...
                            if collection_name:
                                ...
                                logger.warning(f"⚠️  No collection_metadata in result and no collection_name available. Result type: {type(result).__name__}")
                                if isinstance(result, dict):
