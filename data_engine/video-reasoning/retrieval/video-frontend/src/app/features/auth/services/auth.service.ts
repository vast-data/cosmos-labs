...
export class AuthService {
  ...
  login(username: string, password: string, vastHost: string) {
    ...
    this.httpClient.post<LoginResponse>(`${environment.apiUrl}/auth/login`, loginData)
      .subscribe({
        next: (response) => {
          ...
          // Username already fetched from VAST during login - no need for /auth/me
        },
        error: (response) => {
          const errorDetail = response.error?.detail || 'Authentication failed. Please check your credentials.';
          this.state.update((state) => ({ 
            ...state, 
            token: null, 
            status: 'error', 
