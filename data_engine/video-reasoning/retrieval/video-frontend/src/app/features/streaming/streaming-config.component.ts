...
export class StreamingConfigComponent {
  ...
  constructor() {
    ...
    // Then load backend config to pre-fill S3 endpoint and bucket
    this.http.get<any>(`${environment.apiUrl}/config`).subscribe({
      next: (config: any) => {
        const s3Endpoint = config.s3?.s3_endpoint || '';
        const s3Bucket = config.s3?.s3_upload_bucket || '';
      ...
      error: (err: any) => {
        console.error('Failed to load config for pre-fill:', err);
      }
    });
  }
