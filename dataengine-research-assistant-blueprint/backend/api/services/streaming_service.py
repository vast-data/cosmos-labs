...
class StreamingService:
    ...
    def extract_citations(self, function_name: str, result: Any) -> list:
        """Extract citations from retrieve_chunks or hybrid_query tool result"""
        citations = []
        if function_name in ("retrieve_chunks", "hybrid_query"):
    ...
    def extract_chunks(self, function_name: str, result: Any) -> list:
        """Extract actual text chunks from retrieve_chunks or hybrid_query tool result"""
        chunks = []
        if function_name not in ("retrieve_chunks", "hybrid_query"):
            return chunks
    ...
    def create_tool_hook(self, timing_metrics: Dict[str, Any]) -> Callable:
        ...
        def tool_hook(function_name: str, function_call: Callable, arguments: Dict[str, Any]):
            ...
            try:
                ...
                # Debug logging for retrieve_chunks / hybrid_query
                if function_name in ("retrieve_chunks", "hybrid_query"):
                ...
                # Add metadata_query_used for hybrid_query or retrieve_chunks
                if function_name in ("hybrid_query", "retrieve_chunks") and isinstance(result, dict):
                    metadata_query_used = result.get("metadata_query_used")
                    if metadata_query_used:
                        tool_complete_event["metadata_query_used"] = metadata_query_used
                        logger.info(f"🔍 Added metadata_query_used to tool_complete: {metadata_query_used}")
                
                # Add collection metadata for retrieve_chunks only
                # (hybrid_query is analysis-only and does not have collection metadata)
                if function_name == "retrieve_chunks":
                    collection_metadata = None
                    if isinstance(result, dict):
                        collection_metadata = result.get("collection_metadata")
                    
                    # If metadata not found in result, try to get from VastDB
                    ...
                    if collection_metadata:
                        ...
                        logger.info(f"📊 Added collection metadata to tool_complete: {collection_metadata}")
                ...
                if function_name in ("hybrid_query", "retrieve_chunks") and isinstance(result, dict):
                    mq = result.get("metadata_query_used")
                    if mq:
                        tool_ended_event["metadata_query_used"] = mq
                ...
                try:
                    ...
                    if reloaded_session:
                        ...
                        if should_update_title:
                            ...
                            logger.info(f"Set reloaded_session.summary['title'] = {session_title}, full summary: {reloaded_session.summary}")
                        ...
                        if should_update_summary:
                            ...
                            logger.info(f"Set reloaded_session.summary['summary'] = {session_summary_text[:50]}..., full summary: {reloaded_session.summary}")
                        
                        if should_update_title or should_update_summary:
                            # Verify summary is set before upsert
                            logger.info(f"Before upsert_session - reloaded_session.summary: {getattr(reloaded_session, 'summary', 'MISSING')}")
                            agent.db.upsert_session(reloaded_session)
                            # Verify summary is still set after upsert (should be, but let's check)
                            logger.info(f"After upsert_session - reloaded_session.summary: {getattr(reloaded_session, 'summary', 'MISSING')}")
