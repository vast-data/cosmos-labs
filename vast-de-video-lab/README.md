...
### 7. S3 Batch Video Sync Service

The **video-batch-sync** service is a REST API application that performs batch copying of MP4 video files from a source S3 bucket to a destination S3 bucket using server-side copy operations. Files are automatically processed by the ingest pipeline after copying. Supports rate limiting, progress tracking, and works across different S3 endpoints.

**For complete documentation, see:** [video-batch-sync/README.md](video-batch-sync/README.md)
...
- `vastdatasolutions/vde-video-batch-sync:v1` - Video batch sync service
...
- `video-batch-sync-deployment.yaml` - Change `image:` field
...
- **Video Batch Sync Service** - Service for batch copying MP4 files from S3 buckets (see [video-batch-sync/README.md](video-batch-sync/README.md))
...
**What the script does:**
   ...
   - Deploys video batch sync service
